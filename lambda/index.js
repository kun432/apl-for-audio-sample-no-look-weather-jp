...
const GetWeatherHandler = {
    ...
    handle(handlerInput) {
        ...
        const weather = [
            { code: 01, description: '曇り', temp: 28},
            { code: 02, description: '雨',   temp: 35}
        ]
        const id = Math.floor(Math.random() * Math.floor(weather.length));
        console.log(`id: ${id}`);
        ...
        const ssml = requestAttributes.t('WEATHER_REPORT', { currentTemp: weather[id].temp, weatherDescription: weather[id].description });
        let audio = '';
        let bgImage = '';

        // logic to determine which assets should be paired with the forecast
        if (weather[id].code === 01) {
